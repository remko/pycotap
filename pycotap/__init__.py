...
class TAPTestResult(unittest.TestResult):
    ...
    def __init__(self, output_stream, error_stream, message_log,
                 test_output_log):
    ...
    def print_result(self, result, test, directive=None):
        self.output_stream.write("%s %d %s" %
                                 (result, self.testsRun, test.id()))
        if directive:
    ...
    def stopTest(self, test):
        ...
        if self.message_log == self.test_output_log:
            ...
            logs = [(self.test_output_log, self.test_output, "test_output"),
            ...
            if log_mode != LogMode.LogToError:
                ...
                if len(output):
                    if log_mode == LogMode.LogToYAML:
                        ...
                        self.print_raw("  ---\n")
                        self.print_raw("    " + log_name + ":\n")
                        self.print_raw("      File-Name: " + log_name +
                                       ".txt\n")
                        self.print_raw("      File-Type: text/plain\n")
                        self.print_raw("      File-Content: " +
...
class TAPTestRunner(object):
    ...
    def run(self, test):
        result = TAPTestResult(self.output_stream, self.error_stream,
                               self.message_log, self.test_output_log)
        test(result)
        result.printErrors()
